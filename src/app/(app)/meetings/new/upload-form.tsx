...
import { FileAudio, Loader2, ShieldCheck, X } from "lucide-react";
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <div className="space-y-1.5">
        ...
        <Input id="title" name="title" placeholder="Browns — FY2025 year-end planning" required autoFocus />
      </div>
      ...
      {/* Audio upload */}
      <div className="space-y-1.5">
        <Label>Audio / video file (optional)</Label>
        {fileName ? (
          ...
          <label className="flex cursor-pointer items-center justify-center gap-2 rounded-md border border-dashed border-border px-3 py-6 text-sm text-muted-foreground hover:border-primary/40 hover:bg-muted/30">
            {uploading ? (
              <>
              ...
              <>
                <FileAudio className="size-4" /> Choose an mp3, m4a, wav, or mp4 file
              </>
            )}
            <input
              ref={fileInputRef}
        ...
        <p className="text-xs text-muted-foreground">
          No file? You can paste a transcript on the next screen — handy before transcription is configured.
        </p>
